    ...
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            ...
            <div>
                ...
                <p class="m5-5"> {{ $post->descripcion }}</p>
            </div>
                ...
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('posts.destroy', $post) }}" method="POST">
                        ...
                        <input type="submit" value="Eliminar Publicacion"
                            class="bg-red-500 hover:bg-red-600 p-2 rounded text-whote font-bold mt-4 cursor-pointer">
                    </form>
                @endif
            @endauth
        </div>
                ...
                @auth
                    ...
                    <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                        ...
                        <input type="submit" value="Comentar"
                            class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercas font-bold w-full p-3 text-white rounded-lg" />
                    </form>
                @endauth
